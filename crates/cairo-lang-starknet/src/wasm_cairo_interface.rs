...
use crate::{
    ...
    contract::find_contracts,
};
use cairo_lang_compiler::{
    db::RootDatabase, CompilerConfig,
    wasm_cairo_interface::{setup_project_with_input_string, setup_virtual_project, setup_virtual_project_with_deps, init_corelib, DependencyInput},
};
...
pub fn starknet_wasm_compile_with_input_string(
    ...
    let main_crate_inputs = setup_project_with_input_string(&mut db, Path::new("astro.cairo"), input_program_string)?;
...
fn compile_starknet_prepared(
    ...
    // Debug: log registered crates and found contracts
    {
        ...
        let crate_names: Vec<String> = all_crates.iter()
            .map(|c| format!("{:?}", c.long(db)))
            .collect();
    ...
    // Pre-check: capture diagnostics for main crate (and its deps, excluding corelib) to a string
    {
        ...
        let mut check_reporter = cairo_lang_compiler::diagnostics::DiagnosticsReporter::write_to_string(&mut diagnostics)
    ...
    let mut diagnostics_reporter = cairo_lang_compiler::diagnostics::DiagnosticsReporter::callback(|_| ())
        .with_crates(main_crate_inputs);
    if allow_warnings {
    ...
    if output_casm {
        ...
        let sierra_json = serde_json::to_value(&contract)
            .with_context(|| "Sierra serialization failed.")?;
        let casm_json = serde_json::to_value(&casm_contract)
            .with_context(|| "CASM serialization failed.")?;
